...
def recalc(filename, timeout=30):
    ...
    if not setup_libreoffice_macro():
        return {'error': '设置 LibreOffice 宏失败'}
    ...
    # 处理 Linux 和 macOS 之间的超时命令差异
    if platform.system() != 'Windows':
        timeout_cmd = 'timeout' if platform.system() == 'Linux' else None
        if platform.system() == 'Darwin':
            # 检查 macOS 上是否可用 gtimeout
            try:
    ...
    if result.returncode != 0 and result.returncode != 124:  # 124 是超时退出码
        error_msg = result.stderr or '重计算期间发生未知错误'
        if 'Module1' in error_msg or 'RecalculateAndSave' not in error_msg:
            return {'error': 'LibreOffice 宏未正确配置'}
        else:
            return {'error': error_msg}
    
    # 检查重计算文件中的 Excel 错误 - 扫描所有单元格
    try:
        ...
        for err_type, locations in error_details.items():
            if locations:
                result['error_summary'][err_type] = {
                    'count': len(locations),
                    'locations': locations[:20]  # 显示最多 20 个位置
                }
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  - error_summary: 按错误类型分解，包含位置")
