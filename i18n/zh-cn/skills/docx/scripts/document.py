...
按 OOXML schema 顺序放置元素：
        ...
        if track_revisions:
            ...
            if not track_revisions_exists:
                ...
                # 尝试在 documentProtection、defaultTabStop 之前或在开头插入
                inserted = False
  ...
  <{prefix}:rsid {prefix}:val="{self.rsid}"/>
            ...
            # 尝试在 compat 之后、clrSchemeMapping 之前或在结束标签之前插入
            inserted = False
    ...
    def _add_to_comments_xml(
        ...
        # 注意：w:rsidR、w:rsidRDefault、w:rsidP 在 w:p 上，w:rsidR 在 w:r 上，
        # 以及 w:author、w:date、w:initials 在 w:comment 上由 DocxXMLEditor 自动添加
  ...
  <w:p w14:paraId="{para_id}" w14:textId="77777777">
    ...
    def _add_to_comments_extensible_xml(self, durable_id):
        """向 commentsExtensible.xml 添加单个注释。"""
...
<w:r>
  ...
  <w15:presenceInfo w15:providerId="None" w15:userId="{escaped_author}"/>
    ...
    def _ensure_comment_relationships(self):
        """确保 word/_rels/document.xml.rels 有注释关系。"""
        editor = self["word/_rels/document.xml.rels"]
