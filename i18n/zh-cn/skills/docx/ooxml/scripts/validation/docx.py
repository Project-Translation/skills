...
class DOCXSchemaValidator(BaseSchemaValidator):
    ...
    def validate_whitespace_preservation(self):
        """
        验证带有空白的 w:t 元素必须具有 xml:space='preserve' 属性。
        """
        errors = []
                        ...
                        if re.match(r"^\s.*", text) or re.match(r".*\s$", text):
                            ...
                            if (
                                ...
                                errors.append(
                                    ...
                                    f"第 {elem.sourceline} 行: w:t 元素的空白缺少 xml:space='preserve': {text_preview}"
                                )
            ...
            try:
                ...
                for t_elem in problematic_t_elements:
                    if t_elem.text:
                        ...
                        errors.append(
                            f"  {xml_file.relative_to(self.unpacked_dir)}: "
                            f"第 {t_elem.sourceline} 行: <w:t> 在 <w:del> 内: {text_preview}"
                        )
    ...
    def validate_insertions(self):
        ...
        w:delText 只能在 w:ins 中嵌套于 w:del 内时才允许。
        """
        errors = []
        ...
        for xml_file in self.xml_files:
            ...
            try:
                ...
                # 查找 w:ins 中的 w:delText，且不在 w:del 的祖先中
                ...
                for elem in invalid_elements:
                    ...
                    errors.append(
                        f"  {xml_file.relative_to(self.unpacked_dir)}: "
                        f"第 {elem.sourceline} 行: <w:delText> 在 <w:ins> 内: {text_preview}"
                    )
        ...
        if errors:
            ...
            if self.verbose:
                print("通过 - w:ins 元素内未找到 w:delText 元素")
            return True
    ...
    def compare_paragraph_counts(self):
        ...
        print(f"\n段落数量: {original_count} → {new_count} ({diff_str})")
