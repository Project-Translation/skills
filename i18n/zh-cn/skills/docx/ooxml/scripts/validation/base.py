...
class BaseSchemaValidator:
    ...
    # 元素的'id'属性必须在其文件中唯一
    # 格式：元素名 -> (属性名, 范围)
    # 范围可以是 'file'（文件内唯一）或 'global'（跨所有文件全局唯一）
    UNIQUE_ID_REQUIREMENTS = {
        # Word 元素
        "comment": ("id", "file"),  # comments.xml 中的注释 ID
        "commentrangestart": ("id", "file"),  # 必须与注释 ID 匹配
        "commentrangeend": ("id", "file"),  # 必须与注释 ID 匹配
        "bookmarkstart": ("id", "file"),  # 书签开始 ID
        "bookmarkend": ("id", "file"),  # 书签结束 ID
        # 注意：ins 和 del（跟踪更改）在属于同一修订时可以共享 ID
        # PowerPoint 元素
    ...
    def validate_unique_ids(self):
                        ...
                        if id_value is not None:
                            if scope == "global":
                                ...
                                if id_value in file_ids[key]:
                                    ...
                                    errors.append(
                                        f"  {xml_file.relative_to(self.unpacked_dir)}："
                                        f"第 {elem.sourceline} 行：<{tag}> 中的重复 {attr_name}='{id_value}' "
                                        f"(首次出现于第 {prev_line} 行)"
                                    )
                                else:
        ...
        if errors:
            ...
            if self.verbose:
                print("通过 - 所有必需的 ID 唯一")
            return True

    def validate_file_references(self):
        """
        验证所有 .rels 文件正确引用文件，且所有文件都被引用。
        """
        errors = []
        ...
        if self.verbose:
            print(
                f"发现 {len(rels_files)} 个 .rels 文件和 {len(all_files)} 个目标文件"
            )
        ...
        if errors:
            ...
            if self.verbose:
                print(
                    "通过 - 所有引用均有效，所有文件都已正确引用"
                )
            return True

    def validate_all_relationship_ids(self):
        """
        验证 XML 文件中的所有 r:id 属性引用其对应 .rels 文件中存在的 ID，
        并可选地验证关系类型。
        """
        import lxml.etree
                        ...
                        # 检查我们是否对此元素有类型期望
                            ...
                            if expected_type:
                                ...
                                if expected_type not in actual_type.lower():
                                    errors.append(
                                        f"  {xml_rel_path}：第 {elem.sourceline} 行："
                                        f"<{elem_name}> 引用 '{rid_attr}'，它指向 '{actual_type}' "
    ...
    def validate_content_types(self):
        """验证所有内容文件都在 [Content_Types].xml 中正确声明。"""
        errors = []
        ...
        if errors:
            ...
            if self.verbose:
                print(
                    "通过 - 所有内容文件都在 [Content_Types].xml 中正确声明"
                )
            return True

    def validate_file_against_xsd(self, xml_file, verbose=False):
        """针对 XSD 模式验证单个 XML 文件，与原始文件比较。
    ...
    def validate_against_xsd(self):
        """针对 XSD 模式验证 XML 文件，仅显示与原始文件相比的新错误。"""
        new_errors = []
        ...
        if self.verbose:
            ...
            print(
                f"  - 带有新错误：{len(new_errors) > 0 and len([e for e in new_errors if not e.startswith('    ')]) or 0}"
            )
    ...
    def _preprocess_for_mc_ignorable(self, xml_doc):
        ...
        # 在验证前移除 mc:Ignorable 属性
        root = xml_doc.getroot()

        # 从根移除 mc:Ignorable 属性
    ...
    def _validate_single_file_xsd(self, xml_file, base_path):
        """针对 XSD 模式验证单个 XML 文件。返回 (is_valid, errors_set)。"""
    ...
    def _get_original_file_errors(self, xml_file):
        ...
        参数：
            xml_file：要检查的 unpacked_dir 中 XML 文件的路径
        ...
        # 解析两个路径以处理符号链接（例如，macOS 上的 /var 与 /private/var）
        xml_file = Path(xml_file).resolve()
        unpacked_dir = self.unpacked_dir.resolve()
        relative_path = xml_file.relative_to(unpacked_dir)
    ...
    def _remove_template_tags_from_text_nodes(self, xml_doc):
        """从 XML 文本节点移除模板标签并收集警告。

        模板标签遵循 {{ ... }} 模式，用作内容替换的占位符。
        在 XSD 验证前应从文本内容中移除它们，同时保留 XML 结构。
        ...
        for elem in xml_copy.iter():
            ...
            elem.tail = process_text_content(elem.tail, "尾随内容")
