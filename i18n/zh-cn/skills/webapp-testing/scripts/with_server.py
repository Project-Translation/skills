...
def main():
    parser = argparse.ArgumentParser(description='运行命令并启动一个或多个服务器')
    parser.add_argument('--server', action='append', dest='servers', required=True, help='服务器命令（可重复使用）')
    ...
    try:
        ...
        for i, server in enumerate(servers):
            ...
            if not is_server_ready(server['port'], timeout=args.timeout):
                raise RuntimeError(f"服务器在 {args.timeout} 秒内未能在端口 {server['port']} 上启动")
