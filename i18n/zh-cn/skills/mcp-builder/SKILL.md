---
name: mcp-builder
description: 创建高质量 MCP（Model Context Protocol）服务器的指南，使 LLM 能够通过设计良好的工具与外部服务交互。在构建集成外部 API 或服务的 MCP 服务器时使用，无论是 Python（FastMCP）还是 Node/TypeScript（MCP SDK）。
...
创建 MCP（Model Context Protocol）服务器，使 LLM 能够通过设计良好的工具与外部服务交互。MCP 服务器的质量取决于它在多大程度上使 LLM 能够完成现实世界中的任务。
...
**API 覆盖范围与工作流工具：**
在全面的 API 端点覆盖和专门的工作流工具之间取得平衡。工作流工具对于特定任务可能更方便，而全面的覆盖范围使智能体能够组合操作。性能因客户端而异——一些客户端受益于结合基本工具的代码执行，而其他客户端在使用更高级的工作流时工作得更好。不确定时，优先考虑全面的 API 覆盖范围。

**工具命名和可发现性：**
清晰、描述性的工具名称有助于智能体快速找到正确的工具。使用一致的前缀（例如，`github_create_issue`、`github_list_repos`）和以动词为导向的命名。

**上下文管理：**
智能体受益于简洁的工具描述以及过滤/分页结果的能力。设计返回集中、相关数据的工具。一些客户端支持代码执行，这可以帮助智能体过滤和处理数据。

**可操作的错误消息：**
错误消息应通过具体的建议和下一步操作指导智能体。

#### 1.2 学习 MCP 协议文档

**浏览 MCP 规范：**

从站点地图开始，找到相关页面：`https://modelcontextprotocol.io/sitemap.xml`
...
**推荐堆栈：**
- **语言**：TypeScript（高质量的 SDK 支持，与许多执行环境兼容，例如 MCPB。此外，AI 模型擅长生成 TypeScript 代码，得益于其广泛使用、静态类型和良好的代码检查工具）
- **传输**：远程服务器使用可流式 HTTP，使用无状态 JSON（比有状态会话和流式响应更简单，更易于扩展和维护）。本地服务器使用 stdio。
...
**了解 API：**
查看服务的 API 文档，识别关键端点、身份验证要求和数据模型。根据需要使用网络搜索和 WebFetch。

**工具选择：**
优先考虑全面的 API 覆盖范围。列出要实现的端点，从最常见操作开始。
...
### 核心 MCP 文档（首先加载）
- **MCP 协议**：从 `https://modelcontextprotocol.io/sitemap.xml` 开始，然后以 `.md` 后缀获取特定页面
- [📋 MCP 最佳实践](./reference/mcp_best_practices.md) - 包括以下内容的通用 MCP 指南：
  - 服务器和工具命名约定
...
### 评估指南（在第四阶段加载）
- [✅ 评估指南](./reference/evaluation.md) - 包括以下内容的完整评估创建指南：
  - 问题创建指南
  - 答案验证策略
  - XML 格式规范
  - 示例问题和答案
