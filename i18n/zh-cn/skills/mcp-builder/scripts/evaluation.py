...
此脚本通过使用 Claude 对 MCP 服务器运行测试问题来进行评估。
"""
...
当你收到任务时，你必须：
1. 使用可用的工具完成任务
2. 在 <summary> 标签中提供每一步的摘要
3. 在 <feedback> 标签中提供关于工具的反馈
...
反馈要求：
- 在你的 <feedback> 标签中，对工具提供建设性反馈：
  - 评论工具名称：它们是否清晰且描述性强？
  - 评论输入参数：它们是否有良好的文档？必填与可选参数是否清晰？
  - 评论描述：它们是否准确描述了工具的功能？
  - 评论使用工具时遇到的任何错误：工具是否执行失败？工具是否返回了过多的标记？
  - 确定具体的改进领域并解释 WHY 它们会有帮助
  - 在你的建议中要具体且可操作

响应要求：
- 你的响应应该简洁并直接回答所问内容
- 始终在 <response> 标签中包装你的最终响应
- 如果你无法完成任务，返回 <response>NOT_FOUND</response>
- 对于数值响应，仅提供数字
- 对于 ID，仅提供 ID
- 对于名称或文本，提供所请求的确切文本
- 你的响应应该最后出现"""
...
async def evaluate_single_task(
    ...
    print(f"任务 {task_index + 1}：运行问题为 {qa_pair['question']} 的任务")
...
- **平均每任务工具调用数**：{average_tool_calls:.2f}
...
示例：
  ...
  python evaluation.py -t http -u https://example.com/mcp -m claude-3-5-sonnet-20241022 eval.xml
    ...
    remote_group.add_argument("-H", "--header", nargs="+", dest="headers", help="Key: Value 格式的 HTTP 头（仅 sse/http）")
