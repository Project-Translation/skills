...
class BaseSchemaValidator: 
    ...
    # scope 可以为 'file'（文件内唯一）或 'global'（跨文件全局唯一）
    UNIQUE_ID_REQUIREMENTS = {
        # Word 元素
        "comment": ("id", "file"),  # comments.xml 中的注释 ID
        "commentrangestart": ("id", "file"),  # 必须匹配注释 ID
        "commentrangeend": ("id", "file"),  # 必须匹配注释 ID
        "bookmarkstart": ("id", "file"),  # 书签起始 ID
        "bookmarkend": ("id", "file"),  # 书签结束 ID
        # 注意：ins 和 del（修订跟踪）在同一次修订中可以共享 ID

        # PowerPoint 元素
        "sldid": ("id", "file"),  # presentation.xml 中的幻灯片 ID
        "sldmasterid": ("id", "global"),  # 幻灯片母版 ID 必须全局唯一
        "sldlayoutid": ("id", "global"),  # 幻灯片布局 ID 必须全局唯一
        "cm": ("authorid", "file"),  # 注释作者 ID

        # Excel 元素
        "sheet": ("sheetid", "file"),  # workbook.xml 中的工作表 ID
        "definedname": ("id", "file"),  # 命名范围 ID

        # 绘图/形状元素（所有格式）
        "cxnsp": ("id", "file"),  # 连接形状 ID
        "sp": ("id", "file"),  # 形状 ID
        "pic": ("id", "file"),  # 图片 ID
        "grpsp": ("id", "file"),  # 组合形状 ID
    }
    ...
    def validate_xml(self):
        ...
        if errors:
            print(f"失败 - 找到 {len(errors)} 个 XML 违规:")
            for error in errors:
                print(error)
            return False
        else:
    ...
    def validate_unique_ids(self):
        """验证特定 ID 是否符合 OOXML 要求唯一性。"""
        errors = []
        global_ids = {}  # 跟踪所有文件中的全局唯一 ID
        for xml_file in self.xml_files:
            try:
                root = lxml.etree.parse(str(xml_file)).getroot()
                file_ids = {}  # 跟踪此文件中必须唯一的 ID
                            ...
                            if scope == "global":
                                ...
                                if id_value in global_ids:
                                    ...
                                    errors.append(
                                        ...
                                        f"已在 {prev_file} 第 {prev_line} 行的 <{prev_tag}> 中使用"
                                    )
                                else:
                                    global_ids[id_value] = (
        ...
        if errors:
            print(f"失败 - 找到 {len(errors)} 个 ID 唯一性违规:")
            for error in errors:
                print(error)
            return False
        else:
            if self.verbose:
                print("通过 - 所有要求的 ID 均唯一")
            return True

    def validate_file_references(self):
        """验证所有 .rels 文件是否正确引用文件且所有文件都被引用。"""
        errors = []
            ...
            try:
                ...
                if broken_refs:
                    ...
                    for broken_ref, line_num in broken_refs:
                        errors.append(
                            f" {rel_path}: 第 {line_num} 行: 损坏的引用至 {broken_ref}"
                        )
            except Exception as e:
                rel_path = rels_file.relative_to(self.unpacked_dir)
        ...
        if errors:
            print(f"失败 - 找到 {len(errors)} 个关系验证错误:")
            for error in errors:
                print(error)
            print(
                "严重：这些错误将导致文档显示为损坏。"
            ...
            if self.verbose:
                print("通过 - 所有引用有效且所有文件都被正确引用")
            return True

    def validate_all_relationship_ids(self):
        """验证 XML 文件中的所有 r:id 属性是否引用其对应 .rels 文件中存在的 ID，并可选地验证关系类型。"""
        import lxml.etree
                        ...
                        # 检查我们是否有此元素的类型期望
                            ...
                            if expected_type:
                                ...
                                if expected_type not in actual_type.lower():
                                    errors.append(
                                        f" {xml_rel_path}: 第 {elem.sourceline} 行: "
                                        f"<{elem_name}> 引用 '{rid_attr}' 指向 '{actual_type}' "
                                        f"但应指向 '{expected_type}' 关系"
                                    )
            except Exception as e:
                xml_rel_path = xml_file.relative_to(self.unpacked_dir)
        ...
        if errors:
            print(f"失败 - 找到 {len(errors)} 个关系 ID 引用错误:")
            for error in errors:
                print(error)
            print("\n这些 ID 不匹配将导致文档显示为损坏！")
            return False
            ...
            for xml_file in self.xml_files:
                ...
                try:
                    ...
                    if root_name in declarable_roots and path_str not in declared_parts:
                        errors.append(
                            f" {path_str}: 带有 <{root_name}> 根的文件未在 [Content_Types].xml 中声明"
                        )
                except Exception:
                    continue  # 跳过无法解析的文件
            ...
            for file_path in all_files:
                ...
                if extension and extension not in declared_extensions:
                    ...
                    if extension in media_extensions:
                        ...
                        errors.append(
                            f' {relative_path}: 带扩展名 \'{extension}\' 的文件未在 [Content_Types].xml 中声明 - 应添加: <Default Extension="{extension}" ContentType="{media_extensions[extension]}"/>'
                        )
        except Exception as e:
            errors.append(f" 解析 [Content_Types].xml 时出错: {e}")

        if errors:
            print(f"失败 - 找到 {len(errors)} 个内容类型声明错误:")
            for error in errors:
                print(error)
            return False
        else:
            if self.verbose:
                print("通过 - 所有内容文件都在 [Content_Types].xml 中正确声明")
            return True

    def validate_file_against_xsd(self, xml_file, verbose=False):
        """根据 XSD 架构验证单个 XML 文件，与原始文件比较。
        参数:
            xml_file: 要验证的 XML 文件路径
            verbose: 启用详细输出
        返回:
            tuple: (is_valid, new_errors_set) 其中 is_valid 为 True/False/None（跳过）
        """
        # 解析两个路径以处理符号链接
    ...
    def validate_against_xsd(self):
        """根据 XSD 架构验证 XML 文件，仅显示与原始文件相比的新错误。"""
        new_errors = []
        ...
        # 打印摘要
        if self.verbose:
            print(f"验证了 {len(self.xml_files)} 个文件:")
            print(f" - 有效: {valid_count}")
            print(f" - 跳过（无架构）: {skipped_count}")
            if original_error_count:
                print(f" - 带原始错误（忽略）: {original_error_count}")
            print(
                f" - 带新错误: {len(new_errors) > 0 and len([e for e in new_errors if not e.startswith(' ')]) or 0}"
            )
    ...
    def _validate_single_file_xsd(self, xml_file, base_path):
        """根据 XSD 架构验证单个 XML 文件。
        返回 (is_valid, errors_set)。
        """
    ...
    def _get_original_file_errors(self, xml_file):
        """获取原始文档中单个文件的 XSD 验证错误。
        参数:
            xml_file: unpacked_dir 中的 XML 文件路径
        返回:
            set: 原始文件中的错误消息集合
        """
        import tempfile
    ...
    def _remove_template_tags_from_text_nodes(self, xml_doc):
        ...
        在 XSD 验证前应从文本内容中移除，同时保留 XML 结构。
        返回:
