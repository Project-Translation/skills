#!/usr/bin/env python3
"""
将目录打包成 .docx、.pptx 或 .xlsx 文件的工具，同时取消 XML 格式化。

使用示例：
    python pack.py <输入目录> <Office 文件> [--force]
"""
...
def main():
    ...
    parser.add_argument("input_directory", help="未打包的 Office 文档目录")
    ...
    try:
        ...
        # 如果跳过验证，显示警告
        if args.force:
            print("警告：跳过验证，文件可能已损坏", file=sys.stderr)
        # 如果验证失败，退出并报错
        elif not success:
            print("内容将生成损坏的文件。", file=sys.stderr)
            print("请在重新打包前验证 XML。", file=sys.stderr)
            print("使用 --force 可跳过验证并强制打包。", file=sys.stderr)
            sys.exit(1)
